...
class Client(object):
    ...
    def list_members_v2(self):
        ...
        for pool in pools:
            ...
            LOG.debug("DebugMsg(neutron_client)1: %s" % temp_members)
    ...
    def _get_loadbalancer_id_with_listener_id(self, listener_id):
        ...
        LOG.debug("DebugMsg(neutron_client)4: %s" % loadbalancer_id)
    ...
    def list_loadbalancer(self):
        ...
        resources = []
        # LOG.debug("DebugMsg(neutron_client)3: %s(OK)" % "aaaaa")
        if self.lb_version == 'v2':
            # list_loadbalancers works only with lbaas v2 extension
            resp = self.client.list_loadbalancers()
            # LOG.debug("DebugMsg(neutron_client)2: %s(OK)" % resp)
